...
#         search_name = request.form['name'].strip()
#         try:
#             data = pd.read_csv('src/people.csv')
#             # Assuming there is a 'Name' column in your CSV
#             match = data[data['Name'].str.lower() == search_name.lower()]
#             if not match.empty:
#                 first_match = match.iloc[0]
#                 image_url = f"/static/images/{first_match['filename']}"
#         except Exception as e:
#             return f"Error accessing the data: {e}"

#     return render_template('index22.html', image_url=image_url) #search.html
...
def show_image(name):
    ...
    csv_path = 'src/people.csv'  # Adjust the path as needed
